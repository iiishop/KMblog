...
from utility import get_working_github_proxy, get_github_git_url
...
class FrameworkUpdater:
    ...
    def check_for_updates(self):
        ...
        try:
            ...
            # 获取可用的 Git URL（使用代理加速）
            git_url = get_github_git_url()
            ...
            if current_remote.returncode == 0:
                current_url = current_remote.stdout.strip()
                if current_url != git_url:
                    print(f"[更新检查] 切换到代理源: {git_url}")
                    subprocess.run(
        ...
        try:
            ...
            if os.path.exists(old_config_path) and os.path.exists(new_config_path):
                ...
                if new_fields or modified_fields:
                    ...
                    self.log(
                        f"  config.js: 新增 {len(new_fields)} 个字段，修改 {len(modified_fields)} 个字段")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'added' in result.stdout or 'updated' in result.stdout:
                    ...
                    import re
                    match = re.search(
                        r'(added|updated|removed)\s+\d+\s+package', result.stdout)
                    if match:

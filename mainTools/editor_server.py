...
async def track_requests(request, call_next):
    ...
    try:
        ...
        print(
            f"[Middleware-{request_id}] Time before error: {elapsed*1000:.2f}ms")
        print(
            f"[Middleware-{request_id}] ==========================================")
        raise
...
def run_generate_command(operation: str = "operation", async_mode: bool = True):
    ...
    def execute_generate():
        try:
            ...
            print(
                f"[API] Generate command completed successfully in {elapsed:.2f}s")
        except Exception as e:
            print(
                f"[API] Warning: Failed to run Generate command after {operation}: {e}")
            import traceback
            traceback.print_exc()
    ...
    if async_mode:
        # 异步执行，不阻塞API响应
        print(
            f"[API] Scheduling Generate command (async) after {operation}...")
        thread = threading.Thread(
            target=execute_generate, daemon=True, name=f"Generate-{operation}")
        thread.start()
    else:
        # 同步执行
        execute_generate()
...
def run_generate_command_debounced(operation: str = "operation", delay: float = 2.0):
    ...
    def execute_generate():
        try:
            print(
                f"[API] Running Generate command (debounced) after {operation}...")
            import time
            start_time = time.time()
            ...
            elapsed = time.time() - start_time
            print(
                f"[API] Generate command (debounced) completed in {elapsed:.2f}s")
        except Exception as e:
            print(
                f"[API] Warning: Failed to run Generate command (debounced): {e}")
            import traceback
            traceback.print_exc()
...
async def health_check(authorized: bool = Depends(verify_token)):
    ...
    start = time.time()
    print(
        f"[API] HEALTH CHECK - Request received at {time.strftime('%H:%M:%S')}")
...
async def get_file_tree(authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        if not os.path.exists(posts_path):
            ...
            raise HTTPException(
                status_code=404, detail="Posts directory not found")
        ...
        print(
            f"[API] GET FILE TREE - Tree built successfully, {len(tree)} root items")
    ...
    try:
        ...
        def read_file_sync():
            ...
            try:
                ...
                print(
                    f"[{request_id}] [THREAD] File read complete in {read_thread_time*1000:.2f}ms, size: {len(content)} bytes")
                return content
            except Exception as e:
        ...
        def get_version_sync():
            ...
            try:
                ...
                print(
                    f"[{request_id}] [THREAD] Version obtained in {version_thread_time*1000:.2f}ms")
                return version
            except Exception as e:
...
async def move_file(data: MoveFileRequest, authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        from_path = data.from_path or getattr(data, 'from', None) or data.model_extra.get(
            'from') if hasattr(data, 'model_extra') and data.model_extra else None
        to_path = data.to_path or getattr(data, 'to', None) or data.model_extra.get(
            'to') if hasattr(data, 'model_extra') and data.model_extra else None
        ...
        if not os.path.exists(full_from_path):
            raise HTTPException(
                status_code=404, detail=f"Source file not found: {from_path}")
        ...
        source_collection = from_parts[0] if len(
            from_parts) > 1 else 'Markdowns'
        ...
        try:
            ...
            print(
                f"Warning: MovePost command not available, using fallback: {e}")
...
async def rename_file_or_folder(data: RenameRequest, authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        if not os.path.exists(full_path):
            raise HTTPException(
                status_code=404, detail=f"File or folder not found: {data.path}")
        ...
        # 如果是文件，确保新名称有.md扩展名
        if is_file:
            new_name = data.new_name if data.new_name.endswith(
                '.md') else f"{data.new_name}.md"
        else:
...
async def create_folder(data: CreateFolderRequest, authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        parent_full_path = os.path.normpath(
            os.path.join(posts_path, clean_parent_path))
...
async def delete_folder(path: str, authorized: bool = Depends(verify_token)):
    ...
    try:
        ...
        if not os.path.exists(full_path):
            raise HTTPException(
                status_code=404, detail=f"Folder not found: {path}")
    ...
    try:
        ...
        if os.path.exists(target_file_path):
            ...
            while os.path.exists(target_file_path):
                ...
                counter += 1
            print(
                f"[API] FILE UPLOAD - File exists, using new name: {os.path.basename(target_file_path)}")
...
def main():
    ...
    parser.add_argument('--info-file', type=str,
                        help='Path to write server info JSON')
    parser.add_argument('--allow-lan', action='store_true',
                        help='Allow LAN access (bind to 0.0.0.0)')

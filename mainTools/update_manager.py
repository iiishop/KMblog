...
from utility import get_working_github_proxy
...
class ManagerUpdater:
    ...
    def __init__(self):
        self.current_exe = sys.executable if getattr(
    ...
    def check_for_updates(self):
        ...
        try:
            ...
            # 获取可用的代理
            proxy = get_working_github_proxy()
            self.current_proxy = proxy

            # 获取最新 release 信息
            api_url = f"{proxy['api']}/repos/iiishop/KMblog/releases/latest"
            print(f"[管理工具更新] API地址: {api_url}")
                ...
                if platform in asset_name:
                    ...
                    if sys.platform == 'win32' and 'zip' in asset_name:
                        ...
                        download_url = original_url.replace(
                            'https://github.com', proxy['release'])
                        break
    ...
    def download_and_update(self, download_url, asset_name, progress_callback=None):
        ...
        try:
            ...
            report_progress('download', 0.1,
                            f'正在下载 ({total_size / 1024 / 1024:.1f} MB)')
            ...
            if asset_name.endswith('.zip'):
                ...
                result = subprocess.run(
                    ['hdiutil', 'attach', download_path,
                        '-mountpoint', mount_point, '-nobrowse'],
                    capture_output=True,
                    text=True
                )
                ...
                for dir_name in dirs:
                    if dir_name.endswith('.app'):
                        ...
                        exe_path = os.path.join(
                            app_path, 'Contents', 'MacOS', 'KMblogManager')
    ...
    def _create_update_script(self, new_exe, old_exe):
        ...
        if sys.platform == 'win32':
            ...
            script_path = os.path.join(
                tempfile.gettempdir(), 'kmblog_update.bat')
...
"""
            ...
            # macOS .app 更新脚本
            script_path = os.path.join(
                tempfile.gettempdir(), 'kmblog_update.sh')
